...
class JobSeekerJobDetailScreen extends StatefulWidget {
  final CompanyJobDetailResponse data;
  const JobSeekerJobDetailScreen({super.key, required this.data});
...
class _JobSeekerJobDetailScreenState extends State<JobSeekerJobDetailScreen> {
  final JobSeekerServices _api = JobSeekerServices();
  int activeIndex = 0;
  bool isLoading = false;

  void postJob() {
    setState(() => isLoading = true);
    final userId = Provider.of<AccountDataProvider>(context, listen: false)
        .accountDataList[0]
        .id;
    final profileId =
        Provider.of<JobSeekerProfileProvider>(context, listen: false)
            ...
            .id;
    JobApplicationRequest data = JobApplicationRequest(
      jobSeekerProfile: profileId,
      companyJob: widget.data.id,
      applicationStatus: 4,
      createdBy: userId,
      modifiedBy: userId,
      isDelete: false,
    );
    _api.postJobApplication(data).then((response) {
      if (response) {
        successToast('Application submitted successfully');
        GoRouter.of(context).pushNamed(AppRoute.jobSeekerJobApplication);
      } else {
        errorToast('Error submitting application');
      }
  ...
  Widget _buildCompanyInfoCard() {
    return _buildInfoCard(
      ...
      content:
          widget.data.companyProfile?.description ?? 'No description provided',
      children: [
        _InfoRow(
  ...
  Widget _buildJobDetailsCard() {
    return _buildInfoCard(
      ...
      title: widget.data.jobTitle ?? 'Position',
      content: widget.data.jobDescription ?? 'No job description provided',
      children: [
        _InfoRow(
          icon: CupertinoIcons.money_dollar,
          title: 'Salary',
          value: widget.data.salary ?? 'Not specified',
        ),
      ],
    );
  }
      ...
      children: [
        Wrap(
          ...
          children: [
            _InfoChip(
              icon: Icons.schedule,
              label: widget.data.jobType ?? 'Full-time',
              color: AppColor.appPrimaryColor,
            ),
            _InfoChip(
              icon: Icons.engineering,
              label: widget.data.jobField ?? 'General',
              color: AppColor.appSecondaryColor,
            ),
            _InfoChip(
              icon: Icons.star,
              label: widget.data.expLevel ?? 'Entry Level',
              color: AppColor.appPrimaryColor,
            ),
          ],
        ),
  ...
  Widget _buildSkillsCard() {
    return _buildInfoCard(
      ...
      content: widget.data.skills ?? 'No specific skills required',
      children: const [],
    );
  }
  ...
  Widget _buildSubmitButton() {
    return SizedBox(
      ...
      child: isLoading
          ? const CircularProgressIndicator(color: AppColor.appPrimaryColor)
          : BtnWidget(
              btnName: 'Apply Now',
              voidCallback: postJob,
            ),
    );
  }
